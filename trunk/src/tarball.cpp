...
void Tar::_init(void* header)
    ...
    char * s = ::getlogin();
    if(s!=NULL)  std::snprintf(TARHEADER->uname,32,"%s",s);
...
void Tar::putFile(std::FILE* tmpFile,const char* nameInArchive)
	{
	char buff[BUFSIZ];
	std::fseek(tmpFile, 0L, SEEK_END);
	long int len= std::ftell(tmpFile);
	std::fseek(tmpFile,0L,SEEK_SET);
	...
	std::size_t nRead=0;
	while((nRead=std::fread(buff,sizeof(char),BUFSIZ,tmpFile))>0)
		{
